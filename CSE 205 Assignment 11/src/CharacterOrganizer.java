...
public void fromSupportingStackToFinalStack()
{
	if (stackWithNextSmallest >= 0
	    && supportingStacks[stackWithNextSmallest].isEmpty() == false)
	 {
      ...
      char tempChar = supportingStacks[stackWithNextSmallest].pop();
      finalStack.push(tempChar);
   ...
   for (int i = 0; i < numberOfSupportingStacks; i++)
    {
	  ...
	  int num = (int) character2;
	  char letter = (char) (num + 1);
	  boolean bool = false;
	  if(supportingStacks[i].empty() == false && (supportingStacks[i].peek().equals(letter)))
	  {
		  chosenStackIndex = i;
		  break;
	  }	 
	  else if(supportingStacks[i].empty() == false && (supportingStacks[i].peek()).compareTo(character2)>0)
	  { 
		  chosenStackIndex = i;
		  bestTop = supportingStacks[i].peek();		  
		  for(int j = 0; j < numberOfSupportingStacks; j++)
		  {	
			if(supportingStacks[j].empty() == false && ((supportingStacks[j].peek()).compareTo(bestTop) < 0) && (supportingStacks[j].peek() > character2))
			{
				chosenStackIndex = j;
				bool = true;
				break;
